...
class MyRegForm(forms.Form):
    username = forms.CharField(label='用户名',min_length=3,max_length=20,
        error_messages={'register':'用户名不能为空',
                        ...
                        'max_length':'用户名不能多于20位'},
        widget =  forms.widgets.TextInput(attrs={'class':'form-control'})
    )
    password = forms.CharField(label='密码', min_length=3, max_length=20,
        error_messages={'register':'密码不能为空',
                        'min_length':'密码不能少于3位',
                        'max_length':'密码不能多于20位'},
        widget =  forms.widgets.PasswordInput(attrs={'class':'form-control'})
    )
    confirm_password = forms.CharField(label='确认密码', min_length=3, max_length=20,
        error_messages={'register':'确认密码不能为空',
                        'min_length':'确认密码不能少于3位',
                        'max_length':'确认密码不能多于20位'},
        widget =  forms.widgets.PasswordInput(attrs={'class':'form-control'})
    )
